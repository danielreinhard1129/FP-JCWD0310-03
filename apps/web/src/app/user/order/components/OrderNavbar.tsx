...
import { CalendarIcon, ChevronLeft, SlidersHorizontal, X } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col gap-4 container pt-4 bg-white px-6">
      <div className="relative flex gap-2">
        ...
        <div
          className={`absolute flex h-6 right-0 my-auto p-0.5 rounded-md ${openSearch == false ? `` : `bg-slate-300`}`}
        >
          <SlidersHorizontal
            onClick={toggleSearch}
            className={`cursor-pointer h-5 w-5 right-0 my-auto `}
          />
        </div>
      </div>
